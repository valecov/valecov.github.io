# valecov.github.io
# 👾 Página personal de Valentina Covarrubias Faure

![Status](https://img.shields.io/badge/Status-Online-success?style=for-the-badge&color=A5D6A7)
![Style](https://img.shields.io/badge/Style-Pixel%20Art-purple?style=for-the-badge&color=B39DDB)
![Vibe](https://img.shields.io/badge/Vibe-Cozy%20%26%20Pastel-pink?style=for-the-badge&color=FFC4D6)

> **"Hola!"** Este repositorio contiene el código fuente de mi portafolio personal, diseñado con una estética retro de videojuego pero con una paleta de colores suave.

🌐 **Ver Demo:** [https://valecov.github.io](https://valecov.github.io)

---

## 🎮 Sobre el Proyecto

Esta página web sirve como mi carta de presentación profesional. Fue construida con el objetivo de mostrar mis habilidades en **Ciencia de Datos**, reflejando al mismo tiempo mi personalidad y mis intereses personales.

### ✨ Características Principales
* **Diseño 100% Responsivo:** Se adapta a celulares y computadoras.
* **Estilo Pixel Art:** Botones, fuentes y contenedores inspirados en interfaces de RPGs antiguos.
* **Dark/Pastel Mode:** Una combinación de colores amigable a la vista (#FFC4D6, #FFFDD0).
* **Sin Frameworks Pesados:** Hecho con HTML5 y CSS3 puro para máximo rendimiento.

---

## 🛠️ Tech Stack (Herramientas)

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![Git](https://img.shields.io/badge/GIT-E44C30?style=for-the-badge&logo=git&logoColor=white)
![Cursor](https://img.shields.io/badge/Cursor-000000?style=for-the-badge&logo=cursor&logoColor=white)

---

## 🏆 Inventario (Mis Proyectos)

En este portafolio encontrarás detalles sobre mis trabajos más recientes en el ITAM:

| Proyecto | Tech Stack | Descripción |
| :--- | :--- | :--- |
| **CDMX Mortality DB** | SQL, PostgreSQL | Análisis y normalización (4FN) de +120k registros de defunciones. |
| **Pokémon VGC Pipeline** | PySpark, Python | Pipeline de Big Data para analizar el metagame competitivo. |
| **Farah Adventures** | Docker, API | Endless runner conectado a una API de clima en tiempo real. |

---

## 🎨 Créditos y Recursos

* **Fuentes:** "Press Start 2P" y "Roboto Mono" (Google Fonts).
* **Iconos:** FontAwesome / Devicon.
* **Inspiración:** Juegos indie "Cozy" y estética 8-bit.



